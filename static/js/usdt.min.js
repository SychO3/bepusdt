(function () {
    ...
    function handleCopy(text, onVisualFeedback) {
        navigator.clipboard.writeText(text).then(function () {
            if (typeof onVisualFeedback === 'function') {
                onVisualFeedback();
            }
        }).catch(function (err) {
    ...
    function copyAmount() {
        ...
        // 只复制数值部分（去掉币种）
        const amountOnly = text.split(/\s+/)[0] || text;

        handleCopy(amountOnly, function () {
            const originalText = el.textContent;
            el.classList.add('copied');
            el.textContent = originalText + ' · 已复制';
